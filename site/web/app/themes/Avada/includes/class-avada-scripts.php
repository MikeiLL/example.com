...
class Avada_Scripts {
		...
		if ( is_singular() ) {
			...
			if ( $post_type && ! $content_override ) {
				...
				if ( fusion_get_option( $setting_name ) || '' === fusion_get_option( $setting_name ) ) {
					...
					if ( is_rtl() && 'file' !== $this->get_compiler_mode() ) {
	...
	public function combine_stylesheets( $original_styles ) {
		...
		if ( function_exists( 'wpcf7_plugin_path' ) ) {
			$cf7_styles = fusion_file_get_contents( wpcf7_plugin_path( 'includes/css/styles.css' ) );
			if ( function_exists( 'wpcf7_is_rtl' ) && wpcf7_is_rtl() ) {
				$cf7_styles .= fusion_file_get_contents( wpcf7_plugin_path( 'includes/css/styles-rtl.css' ) );
			}
			$cf7_styles = str_replace( '../../assets/ajax-loader.gif', wpcf7_plugin_url( 'assets/ajax-loader.gif' ), $cf7_styles );
