...
class RateLimit {
	...
	public function __construct() {
		...
		$this->allowed_attempts = apply_filters( 'wpforms_stripe_rate_limit_allowed_attempts', 3 );
		$this->expires_in       = apply_filters( 'wpforms_stripe_rate_limit_expires_in', HOUR_IN_SECONDS * 6 );
	}
	...
	private function get_file_path() {
		...
		// Start of compatibility block for wpforms_upload_dir().
		// Can be removed once WPForms 1.6.1 is widely spread.
		if ( empty( $upload_dir['path'] ) ) {
			$upload_dir = wp_upload_dir();
		}

		if ( ! empty( $upload_dir['basedir'] ) ) {
			$upload_dir['path'] = trailingslashit( realpath( $upload_dir['basedir'] ) ) . 'wpforms';
		}
		// End of compatibility block for wpforms_upload_dir().
	...
	private function create_index_html_file() {
		...
		return file_put_contents( $index_file, '' ) !== false; // phpcs:ignore WordPress.WP.AlternativeFunctions
	}
	...
	private function create_file( $file ) {
		...
		if ( file_put_contents( $file, '' ) === false ) { // phpcs:ignore WordPress.WP.AlternativeFunctions
			return false;
		}
	...
	private function write_whole_file( $contents ) {
		...
		return (bool) file_put_contents( $file, wp_json_encode( $contents ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions
	}
